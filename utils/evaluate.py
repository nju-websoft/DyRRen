...
'''
 This script is from FinQA: https://github.com/czyssrs/FinQA/blob/main/code/evaluate/evaluate.py
'''
...
def program_tokenization(original_program):
    ...
    original_program = list(map(str.strip, original_program))
    program = []
...
def evaluate_result(output_dir, examples, json_in, json_ori):
    '''
    execution acc
    program acc
    '''
    ...
    with open(json_ori, 'r', encoding='utf-8') as f_in:
    ...
    for each_example in examples:
        assert each_example.example_id not in retrieval_dict
        retrieval_dict[each_example.example_id] = each_example.texts
    ...
    for each_data in data:
        ...
        #         print(each_ori_data["id"])
        #     prog_correct += 1
        prediction_prog = ','.join(pred).replace('(,', '(').replace(',)', ')')
        each_ori_data["qa"]["predicted"] = prediction_prog

        if exe_res != gold_res:
            res_list.append(each_ori_data)
            error_data = {}
            error_data["filename"] = each_ori_data["id"]
            error_data["retrievals"] = retrieval_dict[each_ori_data["id"]]
            error_data["qa"] = {}
            # error_data["qa"]["question"] = each_ori_data["qa"]["question"]
            error_data["qa"]["program"] = each_ori_data["qa"]["program"]
            error_data["qa"]["predict"] = prediction_prog
